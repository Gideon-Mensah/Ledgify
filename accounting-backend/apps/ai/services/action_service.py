...
def propose_journal(*,organisation,user,conversation,requested_action,payload):
 ...
 for item in payload.get("lines",[]):
  ...
  debit+=d;credit+=c;lines.append({"account_id":str(account.id),"account":{"id":str(account.id),"code":account.code,"name":account.name},"debit":str(d),"credit":str(c),"description":str(item.get("description",""))})
...
def execute_action(*,action,user):
 ...
 require_organisation_permission(organisation=row.organisation,user=user,permission=POST_JOURNAL);payload=row.proposed_payload["payload"];accounts={str(x.id):x for x in Account.objects.filter(organisation=row.organisation,id__in=[line["account_id"] for line in payload["lines"]],status="active")}
