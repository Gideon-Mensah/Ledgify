...
class AISafetyTests(TestCase):
 ...
 def test_financial_question_uses_report_context(self):
  journal=JournalEntry.objects.create(organisation=self.org,entry_number="AI-TEST",date=date(2026,8,1),description="Revenue",status="draft",created_by=self.user);revenue=self.account(self.org,"4000","Sales","revenue","sales");JournalLine.objects.create(journal_entry=journal,account=self.expense,debit=20,credit=0);JournalLine.objects.create(journal_entry=journal,account=revenue,debit=0,credit=100);bank=self.account(self.org,"1000","Cash","asset","bank");JournalLine.objects.create(journal_entry=journal,account=bank,debit=80,credit=0);post_journal_entry(journal,self.user);message=ask_assistant(conversation=self.conversation,question="What was net profit this period?",parameters={"start_date":"2026-08-01","end_date":"2026-08-31"});self.assertIn("80.00",message.content);self.assertEqual(message.metadata["sources"][0]["source_type"],"profit_loss")
