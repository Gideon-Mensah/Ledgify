import { ArrowLeft, Printer, Download, Mail, Pencil, Check, CreditCard, Undo2 } from "lucide-react";
import { invoiceDate, invoiceMoney } from "../../utils/invoicePresentation.js";
...
export default function SourceDocumentPage({ kind }) {
  ...
  return <div className={`source-document-page ${sales ? "invoice-detail-page" : ""}`}>
    <div className="document-actions"><Link to={`/${base}/${collection}`}><ArrowLeft size={16} aria-hidden="true"/>Back to {collection}</Link>
      <button disabled={busy||loading} onClick={print}><Printer size={16} aria-hidden="true"/>Print</button>{actions.pdf.visible&&<button disabled={busy||loading} onClick={download}><Download size={16} aria-hidden="true"/>Download PDF</button>}
      {sales&&actions.email.visible&&<button disabled={busy||!actions.email.enabled} title={actions.email.reason} onClick={()=>setEmailOpen(true)}><Mail size={16} aria-hidden="true"/>Email</button>}
      {actions.edit.visible&&actions.edit.enabled&&<Link to={`/${base}/${collection}/${id}/edit`}><Pencil size={16} aria-hidden="true"/>Edit</Link>}
      {actions.approve.visible&&actions.approve.enabled&&<button className="invoice-primary-action" disabled={busy} onClick={()=>run(async()=>{await service.approve(id);await load();setMessage("Document approved.");})}><Check size={16} aria-hidden="true"/>Approve</button>}
      {actions.pay.visible&&actions.pay.enabled&&<button className={actions.approve.enabled ? "" : "invoice-primary-action"} disabled={busy} onClick={()=>setPaymentOpen(true)}><CreditCard size={16} aria-hidden="true"/>Record payment</button>}
      {actions.reverse.visible&&<button className="invoice-danger-action" disabled={busy||!actions.reverse.enabled} title={actions.reverse.reason} onClick={()=>{setReason("");setCorrection(false);}}><Undo2 size={16} aria-hidden="true"/>Reverse document</button>}
    </div>
    ...
    {sales ? <section className="document-payment-history invoice-payment-history"><h2>Payments</h2>{!data.payments?.length ? <p>No payments have been recorded for this invoice.</p> : <div className="document-table-scroll" role="region" aria-label="Invoice payments" tabIndex={0}><table><thead><tr>{['Date', 'Reference', 'Payment method', 'Account', 'Amount', 'Actions'].map(label => <th scope="col" key={label}>{label}</th>)}</tr></thead><tbody>{data.payments.map(payment => <tr key={payment.id}><td>{invoiceDate(payment.paymentDate)}</td><td>{payment.reference || '—'}<small>{payment.status}</small></td><td>{payment.payment_method || payment.paymentMethod || '—'}</td><td>{payment.bankAccountName || '—'}</td><td>{invoiceMoney(payment.amount, data.currency)}</td><td>{payment.accountingJournal && <Link to={`/accounting/journals/${payment.accountingJournal}`}>View journal</Link>}{auth.hasPermission("reverse_journal") && payment.status !== "reversed" && <button className="invoice-danger-action" disabled={busy} onClick={()=>{setReason("");setCorrection(payment);}}>Reverse payment</button>}<Link to={`/documents/customer-payment/${payment.id}`}>Payment confirmation</Link></td></tr>)}</tbody></table></div>}</section> : <section className="document-payment-history"><h2>Payments</h2>{!data.payments?.length&&<p>No payments recorded.</p>}{data.payments?.map(payment=><article key={payment.id}><span>{payment.paymentDate} · {payment.amount} {data.currency} · {payment.status}</span>{payment.accountingJournal&&<Link to={`/accounting/journals/${payment.accountingJournal}`}>View journal</Link>}{auth.hasPermission("reverse_journal")&&payment.status!=="reversed"&&<button disabled={busy} onClick={()=>{setReason("");setCorrection(payment);}}>Reverse payment</button>}<Link to={`/documents/${sales?"customer-payment":"supplier-payment"}/${payment.id}`}>Payment confirmation</Link></article>)}</section>}
