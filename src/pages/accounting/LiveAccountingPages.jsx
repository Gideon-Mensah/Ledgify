...
function DataTable({ rows, reportTable = false }) {
  ...
  return <><div className={`chart-accounts-table-wrapper${reportTable ? " report-table-wrapper" : ""}`}><table className={`chart-accounts-table${reportTable ? " report-data-table" : ""}`}>
    ...
    <tbody>{pagination.pageRows.map((row, index) => <tr key={row.id || index}>{columns.map((column) => <td className={reportTable ? columnClass(column, safeRows) : ""} key={column}>{isMonetaryField(column) ? formatCurrency(row[column]) : displayValue(row[column])}</td>)}</tr>)}</tbody>
...
export function LiveJournalsPage() {
  ...
  const money = (value, journal) => formatCurrency(value, journal.organisation?.base_currency);
...
function GeneralLedgerDisplay({ ledgers, filters, currency }) {
  ...
  return <>{ledgers.map((ledger) => <section className="general-ledger-panel" key={ledger.account.id}><header className="ledger-report-account-header"><h2><Link className="invoice-number-link" to={`/accounting/accounts/${ledger.account.id}?${accountQuery}`}>{ledger.account.code} · {ledger.account.name}</Link></h2><p>{humaniseReportValue(ledger.account.account_type)} · {({receivable: "Accounts Receivable", payable: "Accounts Payable"}[ledger.account.account_class] || humaniseReportValue(ledger.account.account_class))} · {humaniseReportValue(ledger.account.normal_balance)} normal balance</p></header><div className="ledger-report-scroll" role="region" aria-label={`${ledger.account.code} ledger transactions`} tabIndex={0}><table className="ledger-report-table"><colgroup>{["date","journal","reference","description","source","money","money","balance"].map((name,index)=><col className={`ledger-column-${name}`} key={index}/>)}</colgroup><thead><tr><th scope="col">Date</th><th scope="col">Journal</th><th scope="col">Reference</th><th scope="col">Description</th><th scope="col">Source</th><th scope="col" className="ledger-report-money">Debit</th><th scope="col" className="ledger-report-money">Credit</th><th scope="col" className="ledger-report-money">Running balance</th></tr></thead><tbody><tr className="ledger-opening-row"><th scope="row" colSpan="5">Opening balance</th><td className="ledger-report-money">—</td><td className="ledger-report-money">—</td><td className="ledger-report-money">{money(ledger.opening_balance)}</td></tr>{ledger.transactions.length ? ledger.transactions.map((row, index) => { const sourceRoute = getJournalSourceRoute(row.source_type, row.source_id); return <tr key={`${row.journal_id}-${index}`}><td>{row.date}</td><td><Link to={`/accounting/journals/${row.journal_id}`}>{row.entry_number}</Link></td><td>{row.reference || "—"}</td><td>{row.description || "—"}</td><td>{sourceRoute ? <Link to={sourceRoute}>{journalSourceLabel(row.source_type)}</Link> : journalSourceLabel(row.source_type)}</td><td className="ledger-report-money">{Number(row.debit) ? money(row.debit) : "—"}</td><td className="ledger-report-money">{Number(row.credit) ? money(row.credit) : "—"}</td><td className="ledger-report-money">{money(row.running_balance)}</td></tr>; }) : <tr><td colSpan="8" className="journal-details-empty">No transactions for this account in the selected period.</td></tr>}</tbody><tfoot><tr><td colSpan="5">Period totals</td><td className="ledger-report-money">{money(ledger.total_debit)}</td><td className="ledger-report-money">{money(ledger.total_credit)}</td><td className="ledger-report-money">{money(ledger.balance)}</td></tr></tfoot></table></div></section>)}</>;
}
...
const reportMoney = (value, currency) => {
  return centralFormatCurrency(value, currency, { format: { currencySign: "accounting" } });
};
