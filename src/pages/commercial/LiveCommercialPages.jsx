...
function GenericCommercialDetailPage({ type }) {
  ...
  return <div className="products-page"><PageHeader eyebrow="Commercial workflows" title={document?.quote_number||document?.order_number||document?.purchase_order_number||definition.title} description="Live backend document and lifecycle status." action={<><Link className="invoice-secondary-button" to={definition.base}>Back</Link>{document && <Link className="invoice-secondary-button" to={`/documents/${{quotes:"quote","sales-orders":"sales-order","purchase-orders":"purchase-order"}[type]}/${document.id}`}>Document / Print / PDF</Link>}</>}/>{error&&<div className="invoice-form-alert">{error}</div>}{document&&<><section className="invoice-form-card"><Table rows={[document]} base={definition.base} links={false}/></section><section className="invoice-form-card"><h2>Lines</h2><Table rows={document.lines||[]} base={definition.base} links={false} currency={document.currency}/></section></>}</div>;
}
