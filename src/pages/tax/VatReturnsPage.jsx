...
import { ArrowDownLeft, ArrowUpRight, CalendarDays, CircleHelp, FileText, Printer, RefreshCw, Search, Settings2, SlidersHorizontal } from "lucide-react";
...
export default function VatReturnsPage() {
  ...
  return <div className="tax-workspace">
    <PageHeader eyebrow="Tax workspace" title="Indirect tax" description="Understand your tax position, review activity and prepare for each reporting period." action={<><Link className="invoice-secondary-button" to={settingsLink}><Settings2 size={16}/>Tax settings</Link><button className="invoice-secondary-button" disabled={!ready} onClick={() => window.print()}><Printer size={16}/>Print</button>{hasPermission("export_reports") && <ReportExportMenu title="Indirect Tax Report" rows={ready ? taxExportRows(rows, summary) : []} metadata={metadata} disabled={!ready}/>}</>}/>
