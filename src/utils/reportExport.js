...
export function exportReport(rows, format, title, metadata = {}) {
  ...
  if (format === "pdf") { const document = new jsPDF({ orientation: columns.length > 6 ? "landscape" : "portrait" }); document.setFontSize(16); document.text(title, 14, 16); if (meta) { document.setFontSize(9); document.setTextColor(90); document.text(document.splitTextToSize(meta, document.internal.pageSize.getWidth() - 28), 14, 22); } autoTable(document, { head: [columns], body: body.map((row) => row.map(safeText)), startY: meta ? 26 + document.splitTextToSize(meta, document.internal.pageSize.getWidth() - 28).length * 4 : 22, styles: { fontSize: 8, cellPadding: 2 }, headStyles: { fillColor: [15, 76, 92] }, showHead: "everyPage" }); document.save(`${base}.pdf`); return; }
  const metadataRows = title === "Indirect Tax Report" ? [[title], ...Object.entries(metadata).filter(([, value]) => value).map(([key, value]) => [key.replaceAll("_", " "), value]), []] : [];
