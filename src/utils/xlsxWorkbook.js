...
const number = (value) => { const parsed = parseMonetaryAmount(value); if (parsed === null) throw new Error("Cannot export invalid monetary data."); return parsed; };
...
function financialRows(title, rows, metadata) {
  ...
  if (title === "Profit and Loss") {
    ...
    addTotal(number(totals.net_profit ?? 0) < 0 ? "Net Loss" : "Net Profit", totals.net_profit ?? 0, true);
...
export function createXlsxWorkbook({ title, rows = [], metadata = {} }) {
  ...
  const rowCurrencies = [...new Set(sheetRows.map(row => row.currency).filter(code => code && code !== metadata.currency))];
  const rowXml = sheetRows.map((item, index) => {
    const rowNumber = index + 1; const styles = (item.styles || item.values.map(() => item.style || 0)).map(style => style === 6 && rowCurrencies.includes(item.currency) ? 13 + rowCurrencies.indexOf(item.currency) : style);
  ...
  const widths = Array.from({ length: maxColumns }, (_, index) => `<col min="${index + 1}" max="${index + 1}" width="${title === "Indirect Tax Report" ? [15, 24, 28, 22, 14, 18, 22, 18, 18, 18, 20, 12][index] || 18 : index === 0 ? 42 : index === 1 ? 30 : 18}" customWidth="1"/>`).join("");
  const headerRow = Math.max(1, sheetRows.findIndex(row => row.style === 4) + 1);
  ...
  const styles = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="${1 + rowCurrencies.length}"><numFmt numFmtId="164" formatCode="${xml(currencyNumberFormat(metadata.currency))}"/>${rowCurrencies.map((code, index) => `<numFmt numFmtId="${165 + index}" formatCode="${xml(currencyNumberFormat(code))}"/>`).join("")}</numFmts><fonts count="5"><font><sz val="10"/><name val="Aptos"/></font><font><b/><sz val="11"/><color rgb="FF0E2D3A"/><name val="Aptos"/></font><font><b/><sz val="20"/><color rgb="FF0E2D3A"/><name val="Aptos Display"/></font><font><b/><color rgb="FFFFFFFF"/><name val="Aptos"/></font><font><b/><color rgb="FF067647"/><name val="Aptos"/></font></fonts><fills count="6"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF0F4C5C"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFEAF6F8"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFDCFCE7"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFEE2E2"/></patternFill></fill></fills><borders count="3"><border/><border><bottom style="thin"><color rgb="FFBFD0D7"/></bottom></border><border><top style="double"><color rgb="FF0F4C5C"/></top><bottom style="double"><color rgb="FF0F4C5C"/></bottom></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="${13 + rowCurrencies.length}"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="2" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"><alignment wrapText="1"/></xf><xf numFmtId="0" fontId="3" fillId="2" borderId="0" xfId="0"><alignment wrapText="1"/></xf><xf numFmtId="0" fontId="1" fillId="3" borderId="1" xfId="0"/><xf numFmtId="164" fontId="0" fillId="0" borderId="0" xfId="0"><alignment horizontal="right"/></xf><xf numFmtId="164" fontId="1" fillId="3" borderId="1" xfId="0"><alignment horizontal="right"/></xf><xf numFmtId="0" fontId="1" fillId="3" borderId="2" xfId="0"/><xf numFmtId="164" fontId="1" fillId="3" borderId="2" xfId="0"><alignment horizontal="right"/></xf><xf numFmtId="49" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="4" fillId="4" borderId="1" xfId="0"/><xf numFmtId="0" fontId="1" fillId="5" borderId="1" xfId="0"/>${rowCurrencies.map((code, index) => `<xf numFmtId="${165 + index}" fontId="0" fillId="0" borderId="0" xfId="0"><alignment horizontal="right"/></xf>`).join("")}</cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`;
  return zip({
